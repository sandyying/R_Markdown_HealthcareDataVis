dfA2 <- eventReactive(c(input$selectState_1), { 
  ...
  paramQuery <- data.world::qry_sql(
    ...
    WHERE healthcare_clean.drg_definition IN ('310 - CARDIAC ARRHYTHMIA & CONDUCTION DISORDERS W/O CC/MCC', '303 - ATHEROSCLEROSIS W/O MCC', '897 - ALCOHOL/DRUG ABUSE OR DEPENDENCE W/O REHABILITATION THERAPY W/O MCC', '313 - CHEST PAIN', '301 - PERIPHERAL VASCULAR DISORDERS W/O CC/MCC')
...
dfA22 <- eventReactive(c(input$selectState_1,input$yDataMin__A1_, input$yDataMax__A1_), { 
  if( ! is.na(input$yDataMin__A1_) & ! is.na(input$yDataMax__A1_)) {
    dfA2() %>% dplyr::filter(between(input$yDataMin__A1_, input$yDataMax__A1_))
  }
  else {
    dfA2()
  }
